...
export class UI {
  ...
  static displayWelcome(topScores?: { difficulty: string; score: HighScore }[]) {
    ...
    console.log(chalk.bold.cyan('║') + chalk.bold.yellow('                              TYPE RUSH                                     ') + chalk.bold.cyan('║'));
    console.log(chalk.bold.cyan('╚════════════════════════════════════════════════════════════════════════════╝'));
    console.log('\n');
    ...
    if (topScores && topScores.length > 0) {
      ...
      console.log(chalk.yellow(`    ${best.difficulty.toUpperCase()}: ${best.score.wpm} WPM (${best.score.accuracy}%)`));
    }
    console.log('\n');
  }
  ...
  static displayText(targetText: string, typedText: string, cursorPos: number) {
    ...
    // Show what user has typed so far
    if (typedText.length > 0) {
      console.log(chalk.dim('  Your input:'));
      let typedLine = '  ';
      for (let i = 0; i < typedText.length; i++) {
        if (typedText[i] === targetText[i]) {
          typedLine += chalk.green(typedText[i]);
        } else {
          typedLine += chalk.red(typedText[i]);
        }
      }
      typedLine += chalk.bgGray(' ');  // Show cursor
  ...
  static displayProgress(typedText: string, targetText: string, startTime: number) {
    ...
    console.log();
    console.log(chalk.cyan(`  Progress: ${progress.toFixed(0)}%`) + chalk.gray(` | Time: ${elapsed}s`));
    console.log();
  }
  ...
  static displayResults(stats: GameStats, isNewRecord: boolean = false) {
    ...
    console.log('\n');
    console.log(chalk.bold.cyan('╔════════════════════════════════════════════════════════════════════════════╗'));
    console.log(chalk.bold.cyan('║') + chalk.bold.yellow('                            RESULTS                                         ') + chalk.bold.cyan('║'));
    console.log(chalk.bold.cyan('╚════════════════════════════════════════════════════════════════════════════╝'));
    console.log('\n');
    ...
    const accColor = stats.accuracy >= 95 ? chalk.green : stats.accuracy >= 80 ? chalk.yellow : chalk.red;
  ...
  static displayHighScores(scores: { difficulty: string; topScores: HighScore[] }[]) {
    ...
    console.log('\n');
    console.log(chalk.bold.cyan('╔════════════════════════════════════════════════════════════════════════════╗'));
    console.log(chalk.bold.cyan('║') + chalk.bold.yellow('                         HIGH SCORES                                        ') + chalk.bold.cyan('║'));
    console.log(chalk.bold.cyan('╚════════════════════════════════════════════════════════════════════════════╝'));
    console.log('\n');
  ...
  static displayTimedMode(timeRemaining: number, wordsTyped: number, currentWPM: number) {
    console.log('\n');
    console.log(chalk.bold('  ⏱  TIMED MODE - Type as much as you can!'));
    console.log(chalk.gray('  ' + '─'.repeat(78)));
    console.log();

    const timeColor = timeRemaining <= 10 ? chalk.red : timeRemaining <= 30 ? chalk.yellow : chalk.green;
    console.log(timeColor.bold(`  Time Remaining: ${timeRemaining}s`));
  ...
  static displayMultiplayerLobby(roomId: string, playerCount: number) {
    ...
    console.log('\n');
    console.log(chalk.bold.cyan('╔════════════════════════════════════════════════════════════════════════════╗'));
    console.log(chalk.bold.cyan('║') + chalk.bold.yellow('                        MULTIPLAYER LOBBY                                   ') + chalk.bold.cyan('║'));
    console.log(chalk.bold.cyan('╚════════════════════════════════════════════════════════════════════════════╝'));
    console.log('\n');
  ...
  static displayCustomWordListMenu(lists: string[]) {
    ...
    console.log('\n');
    console.log(chalk.bold.cyan('╔════════════════════════════════════════════════════════════════════════════╗'));
    console.log(chalk.bold.cyan('║') + chalk.bold.yellow('                      CUSTOM WORD LISTS                                     ') + chalk.bold.cyan('║'));
    console.log(chalk.bold.cyan('╚════════════════════════════════════════════════════════════════════════════╝'));
    console.log('\n');
